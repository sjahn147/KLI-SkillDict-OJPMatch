...
2. **구인공고 매칭 시스템 구축**
   * 문제 :
      * 구인공고 텍스트를 읽고 숙련 코드를 매칭해야 함
      * 1,000만 건 구인공고 데이터의 효율적 처리 필요
      * Ollama 기반 RAG 시스템 초기 구현 시 시간당 120건의 낮은 처리 성능
   * 해결 :
      * 시스템 복잡도를 낮추고 실용성에 중점을 둔 경량화 매칭 프로세스 설계
      * 정규식 기반 검색과 유사도 필터링으로 시간당 8만 건 처리 달성(667배 향상)
      * 12코어 병렬 처리와 NAS 기반 자동화로 14.3만 건/시간까지 최적화(1,190배 향상)
